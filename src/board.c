...
void print_board(board_t *board){
    FILE *log_file = fopen("saida.txt", "a"); // Abre o arquivo de log para adicionar dados
    fprintf(log_file, "_____________________________________________________________\n");
    for(int i = 0; i < board->lines; i++){
        for(int j = 0; j < board->columns; j++){
            fprintf(log_file, "|  ");

            // Verifica as combinações de unidades, construções e facções presentes na célula
            if(
        ...
        fprintf(log_file, "|\n");
        fprintf(log_file, "|_____|_____|_____|_____|_____|_____|_____|_____|_____|_____|\n");
    }
    fclose(log_file); // Fecha o arquivo de log após a impressão
}