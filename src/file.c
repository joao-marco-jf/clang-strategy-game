...
int read_all_file(FILE *file) {
    // Abre o arquivo de log para limpar seu conteúdo existente
    ...
    // Determina o vencedor do jogo com base no poder e recursos das facções
    faction_t *winner = factions;
    while (factions != NULL) {
        if ((factions->power > 0 && factions->power > winner->power) ||
            (factions->resources > 0 && factions->resources > winner->resources)) {
            winner = factions;
        }
        factions = factions->next;
    }

    // Abre o arquivo de log para adicionar o vencedor
    log_file = fopen("saida.txt", "a");
    fprintf(log_file, "Vencedor: %s\n", winner->name);
    fclose(log_file);

    // Libera a memória alocada para facções, construções, unidades e alianças
