...
void allocate_alliance_test() {
    ...
    CU_ASSERT_EQUAL(alliance->name, "Aliação 01");  // Testa se 'alliance->name' é "Aliação 01";
    ...
    CU_ASSERT_PTR_NOT_NULL(alliance);               // Testa se 'alliance' não é um ponteiro 'NULL';
    CU_ASSERT_EQUAL(alliance->name, "Aliança 02");  // Testa se 'alliance->name' é "Aliança 02";
    CU_ASSERT_PTR_NULL(alliance->next);             // Testa se 'alliance->next' é 'NULL';
    free(alliance);                                 // Libera memória;
...
void insert_alliance_test(){
    ...
    CU_ASSERT_EQUAL(alliances->name, "Aliação 01"); // Testa se 'alliance->name' é "Aliança 02";
    ...
    CU_ASSERT_EQUAL(alliances->name, "Aliança 02"); // Testa se 'alliance->name' é "Aliança 02";
