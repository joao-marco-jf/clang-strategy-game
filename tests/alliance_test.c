...
void allocate_alliance_test() {
    ...
    alliance = allocate_alliance("Aliança 01");             // 'alliance' recebe o retorno do tipo 'alliance_t' da função 'allocate_alliance;
    CU_ASSERT_PTR_NOT_NULL(alliance);                       // Testa se 'alliance' não é um ponteiro 'NULL';
    CU_ASSERT_STRING_EQUAL(alliance->name, "Aliança 01");   // Testa se 'alliance->name' é "Aliação 01";
...
void insert_alliance_test(){
    alliance_t *alliances = NULL;
    insert_alliance(&alliances, "Aliança 01");                      // Insere uma 'alliance' na lista encadeada 'alliances'
    ...
    CU_ASSERT_STRING_EQUAL(alliances->name, "Aliança 01");          // Testa se 'alliance->name' é "Aliança 01";
    ...
    CU_ASSERT_STRING_EQUAL(alliances->next->name, "Aliança 01");    // Testa se 'alliance->name' é "Aliança 02";
...
void remove_alliance_test(){
    ...
    remove_alliance(&alliances, "Aliança 01");              // Remove um 'alliance' na lista encadeada 'alliances' (LIFO - Last in First out)

    CU_ASSERT_STRING_EQUAL(alliances->name, "Aliança 02");  // Testa se 'alliance->name' é "Aliança 02";
    CU_ASSERT_PTR_NULL(alliances->next);                    // Testa se 'alliances->next' é 'NULL';

    remove_alliance(&alliances, "Aliança 01");              // Remove mais um 'alliance' na lista encadeada 'alliances'
